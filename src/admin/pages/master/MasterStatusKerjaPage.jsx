...
export default function MasterStatusKerjaPage() {
  ...
  const generateNextStatusId = () => {
    ...
    const lastId = lastItem.id || 'ST-KONT-000';
    ...
    return `ST-KONT-${String(nextNumber).padStart(3, '0')}`;
};
  ...
  const handleAddSubmit = async (e) => {
        ...
        // payload yang DIINGINKAN backend
        const payload = {
            id: generateNextStatusId(),         // ✅ contoh: ST-KONT-0004
            nama_status: namaInput.trim(),     // ✅ field yang benar
        };

        try {
            await createStatusKerja(payload);
            setIsAddOpen(false);
            setNamaInput('');
            await loadData();
